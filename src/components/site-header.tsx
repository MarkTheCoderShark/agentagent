...
export function SiteHeader() {
	...
	useEffect(() => {
		// Don't add scroll listener on auth pages since they don't scroll
		if (isAuthPage) {
			setScrolled(false)
			return
		}
	...
	return (
		<header className={cn("z-40 transition-all", isAuthPage ? "relative" : "sticky top-0", scrolled ? "py-2" : "")}> 
			<div
				className={cn(
					"container-width transition-all duration-300",
					isAuthPage 
						? "bg-white/95 ring-1 ring-black/5 shadow-lg backdrop-blur px-4"
						: scrolled
			...
			>
				<div className="flex h-16 items-center justify-between">
					...
					{!(isDashboard || !!session?.user || isAuthPage) && (
										...
										{session?.user ? (
											<>
												<Link href="/profile" className="w-full">
													<Button variant="outline" className="w-full border-[#4527a4] text-[#4527a4] hover:bg-[#4527a4] hover:text-white">Profile</Button>
												</Link>
												<Button className="w-full bg-[#4527a4] hover:bg-[#4527a4]/90 text-white" onClick={() => signOut({ callbackUrl: "/" })}>Sign Out</Button>
											</>
										) : (
											<>
