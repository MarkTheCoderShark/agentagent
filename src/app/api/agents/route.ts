...
export async function POST(request: NextRequest) {
	try {
		...
		// Entitlement: enforce agent count by subscription tier
		const user = await prisma.user.findUnique({ where: { id: session.user.id }, select: { subscriptionTier: true } });
		const limit = getAgentLimitForTier(user?.subscriptionTier);
		if (limit !== null) {
			const count = await prisma.agent.count({ where: { userId: session.user.id } });
			if (count >= limit) {
				return NextResponse.json({ message: "Agent limit reached for current plan" }, { status: 403 });
			}
		}
